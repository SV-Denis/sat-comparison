...
def simplify_clauses(clauses, var, val):
    """Shared clause simplification for DP and DPLL."""
...
def unrelevant_clauses(new_clause, clauses, seen):
    """Check if new_clause is subsumed by any existing clause."""
    for clause in clauses:
        if clause.issubset(new_clause):  # e.g., {x} subsumes {x, y}
            return True
    for clause in clauses:
        if clause.issubset(seen):  # e.g., {x} subsumes {x, y}
            return True
    return False

def resolve(c1, c2):
    """Non-cached resolution for sets."""
    resolvents = []
    for lit in c1:
        if -lit in c2:
...
def resolution_solver(clauses, max_clauses=10000, max_steps=100000):
    ...
    clauses = [frozenset(c) for c in clauses]  # Use frozenset for consistency
    steps = 0
...
def davis_putnam(clauses, max_clauses=10000):
    ...
    assignments = {}
    seen = set(clauses)  # Track seen clauses like in RES
    ...
    while True:
        ...
        # ===== RES-Inspired Variable Elimination =====
        ...
        # Generate resolvents *lazily* (like RES)
        new_clauses = []
        for c1 in pos:
            for c2 in neg:
                for resolvent in resolve(c1, c2):  # Reuse your RES function!
                    if not resolvent:
                        return "DP", False, None  # Empty clause
                    fr = frozenset(resolvent)
                    if fr not in seen and not unrelevant_clauses(fr, clauses, seen):
                        if len(seen) >= max_clauses :
        ...
        # Replace old clauses (like RES, but DP-style)
        clauses = [c for c in clauses if var not in c and -var not in c] + new_clauses

# ======================== DPLL Solver ========================
def dpll(clauses, assignments=None):
    """DPLL with frozensets for memory efficiency"""
    assignments = assignments or {}
    clauses = [frozenset(c) for c in clauses]  # Convert to frozenset
...
def generate_satisfiable_3sat(n_vars, n_clauses):
    """Generate a satisfiable 3-SAT problem by first creating a random assignment"""
        ...
        for alg in alg_chooser:
            try:
                ...
                if alg == "DPLL":
                    ...
                    if isinstance(actual, str):  # Handle "UNKNOWN" case
                        actual = None
